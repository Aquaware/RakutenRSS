...
def fromCsv(filepath):
    ...
    out = []
    for (t, b, a, m, v) in df.values:
        time = datetime.strptime(t, '%Y-%m-%d %H:%M:%S.%f')
        out.append([time, b, a, m, v])
    return out
  
def importFromCsv(stock, filepath):
    data = fromCsv(filepath)
    if data is None:
        print('Import Nothing')
    
    d = data[0]
    [time, b, a, m, v] = d
    tick_table = TickTable(stock, time.year, time.month)
    db = PriceDatabase()
    if not db.isTable(tick_table.name):
        db.create(tick_table)
    db.insert(tick_table, data)
    
    
if __name__ == '__main__':
    stock = 'dji'
    dir_path = 'C://Users/Ikuo/Dropbox/トレード/tick/dji/2020/'
    importFromCsv(stock, dir_path + 'dji_Tick_2020-11-02.csv')
